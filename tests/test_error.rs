use keion_etherscan::error::validation::{
    normalize_address, validate_address, validate_block_hash, validate_tx_hash,
};
...
fn test_error_categories() {
    ...
    assert_eq!(
        EtherscanError::Request("test".to_string()).category(),
        "network"
    );
    assert_eq!(
        EtherscanError::Parse("test".to_string()).category(),
        "parsing"
    );
    assert_eq!(
        EtherscanError::InvalidAddress("bad".to_string()).category(),
        "validation"
    );
    assert_eq!(
...
fn test_retryable_errors() {
    ...
    .is_retryable());
    assert!(EtherscanError::RateLimit {
        retry_after: Some(60),
        message: "limit".to_string()
    }
    .is_retryable());
...
fn test_tx_hash_validation() {
    ...
    assert!(
        validate_tx_hash("0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcde")
            .is_err()
    ); // too short
    assert!(
        validate_tx_hash("0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdeg")
            .is_err()
    ); // invalid hex
